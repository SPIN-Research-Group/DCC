...
class xPU:
    ...
    def _mem_time(self, layer: Layer):
        ...
        if self.name == DeviceType.GPU:
            if layer.type == LayerType.ACT and layer.stage != "RELU":
                exec_time = (
                    0.000000447 *
...
class PIM:
    ...
    def __init__(self, config, scaling_factor):
        ...
        self.attention_table = pd.read_csv(config['RES_ATTENTION_TABLE'])
        self.GEMV_table = pd.read_csv(config['RES_GEMV_TABLE'])
        self.res_label = config["RES_ATTACC_LABEL"]
    ...
    def get_time_from_predictor(self, df, layer:Layer):
        input_dim = layer.k
        output_dim = math.ceil(layer.n / 16) * 16
        nhead = math.ceil(layer.m * layer.numOp / self.num_hbm)
        res = df[(df["#head"] == nhead) & (df["input_dim"] == input_dim) & (df["output_dim"] == output_dim)]

        if len(res) == 1:
            return res[self.res_label].min() / 1000 # ms -> s
        else:
            raise  ValueError(f"Not Find the results in the table for {layer.name}(1, {nhead}, {input_dim}, {output_dim})")

    def get_time_and_energy(self, layer: Layer):
        if layer.type == LayerType.X2G:
            return self._io_time_energy(layer)
        elif layer.type == LayerType.FC:
            time = self.get_time_from_predictor(self.GEMV_table, layer)
            energies = [0, 0, 0, 0, 0, 0]
            return time, energies
        elif layer.type == LayerType.MATMUL:
            if 'score' in layer.name:
                time = self.get_time_from_predictor(self.attention_table, layer)
                energies = [0, 0, 0, 0, 0, 0]
                return time, energies
            else:
