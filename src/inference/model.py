...
class Layer:
    ...
    def __init__(self, stage, name, type, has_weight, dtype, m, n, k, numOp):
        ...
        # if type == LayerType.FC:
        #   print(name, n, m, k, numOp)
...
class Transformer:
    ...
    def build(self, batch, lin, lout, attn_on_hetero=False):
        ...
        for stage in range(1, lout, 1):
            ...
            if self.dcc:
              decoder.append(
                  Layer('gen', 'qkv', LayerType.FC, True, self.dtype, int(self.num_heads *  batch / self.tp) ,
                         3 * int(self.hdim / self.num_heads), self.hdim, 1))
            else:
              decoder.append(
                Layer('gen', 'qkv', LayerType.FC, True, self.dtype, int(batch / self.tp),
            ...
            decoder.append(
                Layer('gen', 'score', LayerType.MATMUL, False, self.dtype, 1,
                      ...
                      int(self.num_heads / self.tp) * batch))
            decoder.append(
                Layer('gen', 'softmax_att', LayerType.SOFTMAX, False, self.dtype,
                      1, lin + stage, 1,
                      int(self.num_heads / self.tp) * batch))
            decoder.append(
            ...
            if self.dcc:
              decoder.append(
                  Layer('gen', 'proj', LayerType.FC, True, self.dtype, int(self.num_heads *  batch / self.tp),
                        self.hdim, int(self.hdim / self.num_heads), 1))
            else:
              decoder.append(
