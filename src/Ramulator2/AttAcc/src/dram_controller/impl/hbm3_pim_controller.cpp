...
class HBM3PIMController final : public IDRAMController, public Implementation {
  ...
  public:
    ...
    bool send(Request& req) override {
      ...
      // {
      //   printf("mclk=%ld\n", m_clk);
      //   printf("REQ(%d): addr=0x%08x, addr_vec=0x%08x\n", req.type_id, req.addr, req.addr_vec);
      // }
    ...
    void tick() override {
      ...
      if (request_found) {
        // printf("REQ(%d): addr=0x%08x, 4e\n", req_it->type_id, req_it->addr);
        ...
        if (req_it->command == req_it->final_command) {
          if (req_it->type_id == Request::Type::Read) {
            ...
            // puts("HBM3-PIM Write request received");
          }
      ...
      if (sec_request_found) {
        ...
        if (sec_req_it->command == sec_req_it->final_command) {
          if (sec_req_it->type_id == Request::Type::Read) {
            ...
            // puts("HBM3-PIM-toick write request received");
          }
      ...
      if (request_found) {
        if (m_dram->m_command_meta(req_it->command).is_closing) {
          ...
          for (auto _it = m_active_buffer.begin(); _it != m_active_buffer.end(); _it++) {
            // if (_it == req_it && req_it->type_id == 1)
            //   continue;
