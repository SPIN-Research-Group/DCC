...
class HBM3PIMController final : public IDRAMController, public Implementation {
  ...
  public:
    ...
    bool send(Request& req) override {
      ...
      //FIX PIM_BARRIER
      if (req.type_id == Request::Type::PIM_BARRIER){
        if (m_active_buffer.size() > 0 || m_pim_buffer.size() > 0 || m_read_buffer.size() > 0 || m_write_buffer.size() > 0){
          req.arrive = -1;
          return false;
        }
      }
      ...
      switch (req.type_id) {
        ...
        case Request::Type::PIM_DUMMY:      is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_LD_OP1:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_LD_OP2:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_LD_OP3:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_MUL_OP1:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_MUL_OP2:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_MUL_OP3:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_ADD_OP1:     is_success = m_pim_buffer.enqueue(req);   break; 
        case Request::Type::PIM_ADD_OP2:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_ADD_OP3:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_MAC_OP1:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_MAC_OP2:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_MAC_OP3:     is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_WB_ACC:    is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_WB_RES:    is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_ACC_RESET: is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_RELU:      is_success = m_pim_buffer.enqueue(req);   break;
  ...
  private:
    ...
    int get_command_type(int command) {
      ...
      else if (command == m_dram->m_commands("DUMMY")) return 1; // Col command
      else if (command == m_dram->m_commands("LD_OP1"))  return 1; // Col command
      else if (command == m_dram->m_commands("LD_OP2"))  return 1; // Col command
      else if (command == m_dram->m_commands("LD_OP3"))  return 1; // Col command
      else if (command == m_dram->m_commands("MUL_OP1"))  return 1; // Col command
      else if (command == m_dram->m_commands("MUL_OP2"))  return 1; // Col command
      else if (command == m_dram->m_commands("MUL_OP3"))  return 1; // Col command
      else if (command == m_dram->m_commands("ADD_OP1"))  return 1; // Col command
      else if (command == m_dram->m_commands("ADD_OP2"))  return 1; // Col command
      else if (command == m_dram->m_commands("ADD_OP3"))  return 1; // Col command
      else if (command == m_dram->m_commands("MAC_OP1"))  return 1; // Col command
      else if (command == m_dram->m_commands("MAC_OP2"))  return 1; // Col command
      else if (command == m_dram->m_commands("MAC_OP3"))  return 1; // Col command
      else if (command == m_dram->m_commands("WB_ACC"))   return 1; // Col command
      else if (command == m_dram->m_commands("WB_RES"))   return 1; // Col command
      else if (command == m_dram->m_commands("ACC_RESET"))return 1; // Col command
      else if (command == m_dram->m_commands("RELU"))     return 1; // Col command
