...
# print(HBM_GS['ba'] / 1024 / 1024)  # 16MB
# print(HBM_GS['hbm'] / 1024 / 1024)  # 16GB
...
def gen_half_bank_va_cmds(T, Vec1_addr, Vec2_addr, Ret_addr, banks_bases, pos_i_st):
  cmds = []
  for pos_i_grf_st in range(0, math.ceil(T[1][2] /  n_mac), n_grf):
    ...
    for pos_i in range(pos_i_grf_st, pos_i_grf_end):
      for bank_base in banks_bases:
        addr = Vec1_addr + bank_base + (pos_i_st +pos_i) * data_size
        hex_addr = hex(addr)[2:]
        cmds.append("PIM_LD_OP1 0x{0:0>8}".format(hex_addr))
    for pos_i in range(pos_i_grf_st, pos_i_grf_end):
      for bank_base in banks_bases:
        addr = Vec2_addr + bank_base + (pos_i_st +pos_i) * data_size
        hex_addr = hex(addr)[2:]
        cmds.append("PIM_ADD_OP1 0x{0:0>8}".format(hex_addr))
    for pos_i in range(pos_i_grf_st, pos_i_grf_end):
      for bank_base in banks_bases:
        addr = Ret_addr + bank_base + (pos_i_st +pos_i) * data_size
        hex_addr = hex(addr)[2:]
...
def VA(T, Vec1_addr, Vec2_addr, Ret_addr, barrier):
  T = match_alignment(T)
  cmd_ld = []
  ...
  for pos in range(math.ceil(T[0][2] * T[1][2] / n_mac)):
    for ba_idx in range(T[0][1] * T[1][1]):
      for lch_idx in range(T[0][0] * T[1][0]):
        addr = Vec2_addr + lch_idx * HBM_GS['ch'] + ba_idx * HBM_GS['ba'] + pos * prefetch_size
        hex_addr = hex(addr)[2:]
        cmd_ld.append("LD 0x{0:0>8}".format(hex_addr))
  cmd_ld += barrier
  ...
  for itr in range(T[0][2]):
    pos_i_st = itr * T[1][2]
    cmd_va += gen_half_bank_va_cmds(T, Vec1_addr, Vec2_addr, Ret_addr, even_bank_base, pos_i_st)
    cmd_va += gen_half_bank_va_cmds(T, Vec1_addr, Vec2_addr, Ret_addr, old_banks_base, pos_i_st)
...
def total_size(tiling, num_itr):
  ...
  return itr_size, Vec1_size


def run_VA(tiling, output_path):
  num_itr = 1
  itr_size, Vec_size = total_size(tiling, num_itr)

  while num_itr <= tiling[0][2]:
    itr_size, Vec_size = total_size(tiling, num_itr)
    if Vec_size * 3 <= HBM_GS["ba"]:
      break
    else:
      num_itr += 1
  ...
  Vec1_addr = 0
  Vec2_addr = Vec1_addr + Vec_size
  Ret_addr = Vec2_addr + Vec_size
  ...
  for itr in range(num_itr):
    va_cmd = VA(real_tiling, Vec1_addr, Vec2_addr, Ret_addr, barrier)
    total_cmd += va_cmd
  # return
...
def gen_single_name(config_info, output_dir="./traces"):
  ...
  output_path = f"{output_dir}/va({batch}, {vec_len})_[{tiling[0]}-{tiling[1]}]_bank.trace"
  return output_path
