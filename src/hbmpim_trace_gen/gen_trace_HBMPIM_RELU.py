...
def gen_half_bank_relu_cmds(T, Vec1_addr, Ret_addr, banks_bases, pos_i_st):
  cmds = []
  for pos_i_grf_st in range(0, math.ceil(T[1][2] /  n_mac), n_grf):
    ...
    for pos_i in range(pos_i_grf_st, pos_i_grf_end):
      for bank_base in banks_bases:
        ...
        hex_addr = hex(addr)[2:]
        cmds.append("PIM_RELU 0x{0:0>8}".format(hex_addr))
    for pos_i in range(pos_i_grf_st, pos_i_grf_end):
      for bank_base in banks_bases:
...
def RELU(T, Vec1_addr, Ret_addr, barrier):
  T = match_alignment(T)
  cmd_ld = []
  ...
  for itr in range(T[0][2]):
    pos_i_st = itr * T[1][2]
    cmd_relu += gen_half_bank_relu_cmds(T, Vec1_addr, Ret_addr, even_bank_base, pos_i_st)
    cmd_relu += gen_half_bank_relu_cmds(T, Vec1_addr, Ret_addr, old_banks_base, pos_i_st)
  ...
  All_cmd = cmd_ld + cmd_relu  + cmd_ret
  return All_cmd
...
def run_RELU(tiling, output_path):
  num_itr = 1
  ...
  for itr in range(num_itr):
    va_cmd = RELU(real_tiling, Vec1_addr, Ret_addr, barrier)
    total_cmd += va_cmd
...
def gen_single_name(config_info, output_dir="./traces"):
  ...
  output_path = f"{output_dir}/relu({batch}, {vec_len})_[{tiling[0]}-{tiling[1]}]_bank.trace"
  return output_path
